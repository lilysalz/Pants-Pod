...
from models import CommentsIn, CommentsOut, EpisodeCommentsList, DeleteStatus
...
@router.post("/api/episodes/comments/{episode_id}", response_model=CommentsOut)
def create_comments(
    comments: CommentsIn,
    account_data: dict = Depends(authenticator.get_current_account_data),
    repo: CommentsRepo = Depends(),
):
    return repo.create_comments(comments=comments, user_id=account_data["id"])
...
def delete_comments(
    ...
    return {
        "success": repo.delete_comments(
            episode_id=episode_id, id=id, user_id=int(account_data["id"])
        )
    }
