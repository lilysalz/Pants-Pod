    ...
    async def update_episode_data(self):
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                # Using the access token to get track info
                if access_token:
                    track_info_url = "https://api.spotify.com/v1/shows/2fB0mLyJDGBFUUyRQTXb9O/episodes?offset=0&limit=20&market=US&locale=en-US,en;q=0.5"
                    track_info_headers = {"Authorization": f"Bearer {access_token}"}
            ...
            print(e)
            return {'message': "Something went wrong with the Spotify Data Query."}
